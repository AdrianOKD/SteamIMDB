...
app.get("/api/games", async (req, res) => {
  ...
  try {
    ...
    const response = await axios({
      ...
      data: `fields name,cover.url,summary,rating,first_release_date,genres.name,platforms.name,screenshots.url,total_rating,storyline,age_ratings.*,type,involved_companies.*,involved_companies.company.*; 
             where cover != null & category = 0 & involved_companies.company.name != null; 
             limit ${limit}; 
             sort rating desc;`
    });
...
app.get("/api/games/:id", async (req, res) => {
  try {
    ...
    const response = await axios({
      ...
      data: `fields name,cover.url,summary,storyline,rating,first_release_date,genres.name,platforms.name,screenshots.url,videos.video_id,similar_games.name,similar_games.cover.url, game_types.type;
             where id = ${id};`
    });
